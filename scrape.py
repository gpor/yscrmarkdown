...
def get_scrape_files():
    if not SCRAPERS_DIR.exists():
        print(f"Error: {SCRAPERS_DIR} directory not found!")
        return []
    
    files = []
    for file in SCRAPERS_DIR.iterdir():
...
def run_selected_script(filename):
    ...
    script_path = SCRAPERS_DIR / filename
...
def main():
    ...
    if not files:
        print(f"No Python files found in the {SCRAPERS_DIR} directory.")
        return
